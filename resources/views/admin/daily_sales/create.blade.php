    ...
    <div class="main-content">
        <div class="page-content">
            <div class="container-fluid">
                ...
                <form class="custom-validation" id="store-daily-sales" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card">
                                        ...
                                        <div class="row product-list" id="productList-{{ $category->id }}">
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-check d-flex justify-content-between align-items-center">
                                                    ...
                                                    <input type="hidden" class="form-control w-25 current-stock" value="{{ $product->stock }}" name="current_stock[]" data-stock-id="{{ $product->id }}">
                                                    <input type="number" class="form-control w-25 quantity-input" value="" name="quantity[]" placeholder="{{ $product->stock }}" data-checkbox-id="product-{{ $product->id }}">
                                                </div>
                                            </div>
                                            @endforeach
                                        </div>
    ...
    <script>
        ...
        $('.product-checkbox').change(function() {
            if($(this).is(':checked')) {
                ...
                $('input.quantity-input[data-checkbox-id="' + checkboxId + '"]').focus();
            }
        });

        $('#truck_data').on('change',function(){
            $.ajax({
                url:"{{ route('daily-sales.get-truck-products') }}",
                type:'POST',
                data:{"truck_id":$(this).val(),"_token":"{{ csrf_token() }}"},
                success:function(resp){
                    ...
                    if (resp) {
                        ...
                        $.each(jsonData, function(index, item) {
                            $('input.old-stock[data-input-id="' + item.product + '"]').val(item.quantity);
                            $('input.product-checkbox#product-' + item.product).prop('checked', true).prop('disabled', true);
                        });
                    }
                }
            });
        ...
        $('#store-daily-sales').on('submit', function(event) {
            ...
            $('.product-checkbox:checked').each(function() {
                ...
                const currentStockInput = $(`input.current-stock[data-stock-id="${productId}"]`);

                selectedProducts.push({
                    product_id: productId,
                    quantity: quantityInput.val(),
                    closing_stock: closingStockInput.val(),
                    current_stock: currentStockInput.val()
                });
            });
            // console.log(selectedProducts);

            $.ajax({
                url: "{{ route('daily-sales.store') }}",
                method: 'POST',
